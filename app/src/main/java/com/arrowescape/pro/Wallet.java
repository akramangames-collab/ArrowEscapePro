...
/** One wallet shared by gameplay, streak rewards, daily challenges and earned-ad callbacks. */
public final class Wallet {
    public static final int STARTER = 100, HINT_COST = 25, DAILY = 10, AD_REWARD = 75;
    public static final int DAILY_CHALLENGE = 200;
    public static final int HEART_COST = 40, CONTINUE_COST = 60;
    public static final long DAY_MS = 86400000L;
    private static final int[] DAILY_STREAK_REWARDS = {10, 15, 20, 30, 40, 50, 75};
    ...
    public static final class State {
        ...
        public State copy() {
            State s = new State();
            s.coins = coins; s.streak = streak; s.arrowTypeMask = arrowTypeMask; s.themeMask = themeMask;
            s.dailyDay = dailyDay; s.challengeDay = challengeDay; s.weeklyChallengeWeek = weeklyChallengeWeek;
    ...
    public static int dailyRewardForStreak(int streakDay) {
        ...
        int index = Math.min(streakDay, DAILY_STREAK_REWARDS.length) - 1;
        return DAILY_STREAK_REWARDS[index];
    }

    public synchronized int nextDailyStreak(long now) {
        state = storage.load().copy();
        if (now < 0) return 1;
        long day = now / DAY_MS;
        if (day <= state.dailyDay) return Math.max(1, state.streak);
        return state.dailyDay == day - 1 ? Math.min(9999, state.streak + 1) : 1;
    }

    public synchronized int nextDailyReward(long now) {
        return dailyRewardForStreak(nextDailyStreak(now));
    }
    ...
    public synchronized boolean canClaimDaily(long now) {
        state = storage.load().copy();
        return now >= 0 && now / DAY_MS > state.dailyDay;
    }
    public synchronized boolean canRewardDailyChallenge(long day) {
        state = storage.load().copy();
        return day >= 0 && day > state.challengeDay;
    }
    public synchronized boolean canRewardWeeklyChallenge(long week) {
        state = storage.load().copy();
        return week >= 0 && week > state.weeklyChallengeWeek;
    }
    ...
    public synchronized int claimDaily(long now) {
        ...
        next.dailyDay = day;
        int amount = dailyRewardForStreak(next.streak);
    ...
    public synchronized int rewardDailyChallenge(long day, int stars) {
        ...
        State next = state.copy();
        next.coins = add(next.coins, DAILY_CHALLENGE);
        next.challengeDay = day;
        return commit(next) ? DAILY_CHALLENGE : 0;
    }
    ...
    private static int add(int balance, int amount) {
