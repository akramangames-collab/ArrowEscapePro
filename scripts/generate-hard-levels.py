...
for level,line in enumerate((ROOT/'tests/v15-levels.txt').read_text().splitlines(),1):
    ...
    target=max(48+int(fraction*140), len(original)+30)
    w=max(original_w+4,22+int(fraction*12));h=max(original_h+4,26+int(fraction*16))
    ...
    while len(pieces)<target:
        ...
        for attempt in range(100):
            ...
            for segment in range(rng.randrange(3,7)):
                if segment:
            ...
            score=sum(bool(taken&rays[i]) for i in available)*100+sum(bool(taken&r) for r in rays)*3+len(taken)*.3+rng.random()
